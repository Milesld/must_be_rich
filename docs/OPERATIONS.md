...
| `research/pick_stocks.py` | 多池子快速选股 — 看今天该买什么 | ✓（拉数据） | ✗ |
...
不需要跑完整回测，用 `pick_stocks.py` 直接输出今日持仓建议：
...
```
======================================================================
  半导体池 — top_2 | 资金 ¥100,000
======================================================================
  代码     名称                    评分       收盘价      建议股数        预估金额
  ──────── ──────────────────── ──────── ────────── ────────── ──────────────
  002371   北方华创                 0.9234    ¥285.50        300      ¥85,650
  300308   中际旭创                 0.8912    ¥142.30        700      ¥99,610

======================================================================
  机器人池 — top_2 | 资金 ¥100,000
======================================================================
  代码     名称                    评分       收盘价      建议股数        预估金额
  ──────── ──────────────────── ──────── ────────── ────────── ──────────────
  300124   汇川技术                 0.8765    ¥58.20       1,700      ¥98,940
  002008   大族激光                 0.8011    ¥32.80       3,000      ¥98,400

======================================================================
  合计预估投入: ¥382,600
```

**工作原理**：加载策略配置 → 拉最新数据 → 计算所有因子 → 按权重排名 → 输出 top-N 和按等资金分配的建议股数。因子计算基于昨天及之前的数据，不包含今天（T+1 前视偏差保护）。

**和回测的区别**：`pick_stocks.py` 只看最近一个交易日、从零开始选股、不考虑 regime 仓位缩放。`run_backtest_demo.py` 是完整的历史回测，包含持仓状态变化、regime 仓位管理、交易成本。

**自定义池子**：编辑脚本中的 `tasks` 列表即可增删池子或调整预算：
...
tasks = [
    ...
    # 添加更多:
    # ("苹果链", "configs/strategy_apple_chain.yaml", 2, 50_000),
]
```

**建议股数说明**：以最近交易日收盘价为参考，取 100 股整数倍。实盘以当天开盘/买入时的实际成交价为准，股数可能略有调整。

**⚠ 注意事项**：
- 建议晚上 20:00 以后跑（新浪数据已更新到当日）
- 如果 regime 评分 < 0.40（紧急减仓触发），应等比减少买入资金
- 月末最后一个交易日跑的结果 = 下月调仓清单
