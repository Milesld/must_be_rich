...
| # | 状态 | 事项 | 内容细节 | 为什么排这个位置 |
|---|------|------|----------|------------------|
| 1 | ✅ 2026-07-15 | 修涨跌停板块配置 bug + 封板检查 | 1.1：板块配置注入引擎（或内置前缀默认表）；1.5：封板判断改为收盘价≈涨跌停价 | 所有历史回测结果失真，修好才有可信的基线；**修完需重跑各池回测刷新结论**（对比数据见附录 B） |
| 2 | ⬜ | 修数据正确性问题 | 1.2：qfq 缓存段缝连续性校验（或改存不复权+复权因子）；1.3：估算日历不落盘/打标记强制刷新 | 数据正确性 > 一切；这两个问题会静默污染未来所有结果 |
| 3 | ⬜ | 修一致性与测试 | 1.6：pick_stocks 接入 westock 财报；1.4：滑点口径统一；1.7：修 merge_asof dtype + `brew install libomp` | 保证「回测的策略 = 实盘跑的策略」，测试全绿 |
| 4 | ⬜ | 搭 paper trading 闭环 | 每日信号 → 虚拟组合账本（JSON/DuckDB）→ 次日真实价格更新净值 → 周度对账报告（模拟 vs 回测预期） | 用最低成本验证「回测≈现实」，是实盘前的必要闸门 |
