...
| 4 | ✅ 2026-07-15 (6190649) | 搭 paper trading 闭环 | `research/paper_trading.py`：`signal`（收盘后生成调仓信号，与回测/pick_stocks 同一套打分）→ `settle`（次日**开盘价**撮合，开盘涨停拒买/跌停拒卖/停牌顺延超 10 交易日撤单，费用含滑点与回测同口径）→ `report`（对账报告）。★双账本设计：真实账本按次日开盘成交 vs 影子账本按信号日收盘无条件成交（=回测的乐观假设），两条净值曲线之差=回测隐藏的执行偏差（隔夜跳空/封板/停牌）。账本 JSON 落 `data/paper_trading/`。14 个单元测试（`test_paper_trading.py`）。简化项：无 regime 仓位缩放、订单无部分成交 | 用最低成本验证「回测≈现实」，是实盘前的必要闸门 |
| 5 | ✅ 2026-07-20 (049f482) | 因子 IC 检验层 | `research/factor_ic.py`：月度截面 RankIC/ICIR/t值/正率 + IC 衰减（1/3/6 月）+ 五分层回测（spread/单调性），方向调整口径（反向因子 adj_ic=−raw_ic，好因子 ICIR 恒为正）；与回测同一套 feature_loader/PIT 财报/动态宇宙。报告落 `data/reports/factor_ic/ic_{pool}.json`；optimizer 新增 `--ic-report`/`--min-icir` 门禁（ICIR 不达标的因子不进搜索空间，未检验的保留并提示）。21 个单元测试。★首跑结论（36 个月度截面，阈值 0.3）：银行池 5 过（volatility_20d .60 / amplitude_20d .56 / atr_14 .39 / bollinger .36 / rsi_14 .34，全是低波动/反转类）；半导体池仅 atr_14 过（.32）——动量类因子在两池 ICIR 均为负，此前优化器搜出的动量组合基本是过拟合。残留：westock 股本接口当日故障 → pb/pe/peg 恒 0 被恒值防护跳过（截面数=0），待接口恢复补跑；gross_margin/net_margin_ttm/net_profit_yoy 同样截面=0（财报缓存字段覆盖不足），补跑时一并核查 | 从「搜过拟合」转向「组合已验证的信号」 |
| 6 | ✅ 2026-07-20 (955595c) | 本地数据仓 + 每日成分快照 | `research/warehouse.py`（Parquet 单股单文件 + kline_meta.json 覆盖速查）+ `scripts/update_data.py`（update/import-cache/status）。★复权基线错位根治换了方案：westock 无不复权/复权因子接口（bfq/hfq 不可用），改为「增量重叠比对」——每次更新多拉最近 15 天与仓内重叠日比对 close，偏差 >0.5% 判定除权基线变化 → 全量重拉该股（机制上可检出段缝启发式漏掉的小额分红错位）。`_load_real_data` westock 分支 warehouse-first：仓内覆盖的代码读本地 parquet，未覆盖才联网（次新股用 fetch_start 元数据区分「上市晚」与「缺数据」）。成分快照 `snapshot_constituents`/`constituents_asof`（≤日期最近快照），update 时自动存，攒一年成自建 PIT 成分库。18 个单元测试。实测：import-cache 从 westock 段缓存引导入仓 102 只/8.9 万行；银行池全区间加载 42/42 只命中本地、联网 0 只、~6 秒（当日 westock 网络故障仍照常服务）。残留：快照攒够历史前 `_build_monthly_universe` 仍用当前成分；`universe.py` 接入 constituents_asof 待快照积累后做 | 长期解决幸存者偏差和限流，回测提速 |
| 7 | ✅ 2026-07-20 (20382c0) | 扩宇宙 + 中性化 + 组合约束 | `research/portfolio_rules.py`（纯函数，回测/pick_stocks/paper_trading 三处共用同一 `select_target_portfolio`）：①rank buffer 换手控制（新买须进 top_n、已持有跌出 buffer 才卖——路线图「换手率上限」的等权 top-N 等效实现）；②单行业只数上限 `max_per_sector`（等权下 k/top_n≈敞口上限，30%→top_n=20 配 6）；③单票权重上限 `max_weight_per_stock`（超额留现金不重分配）。中性化：`_score_stocks` 接入 neutralizer.py（MAD 缩尾→行业/市值回归取残差再排名），feature_loader 新增 `_mktcap` 辅助列（close×westock 股本）。全部配置驱动默认关闭（`strategy.rank_buffer/max_per_sector/max_weight_per_stock/neutralize.{industry,mktcap}`），未配置零行为漂移（313 测试全绿验证）。★扩宇宙调整：中证800 需指数成分接口（westock 无），改为多申万板块并集+pool_size 100（`configs/strategy_multi_sector_westock.yaml` 模板，top_n=20+全约束开启+IC 通过因子），板块码核对待 westock 限流恢复。银行池 A/B 实测（仓内数据，2023-2025）：buffer6+市值中性 vs 基线 → 成交 188→156 笔(−17%)，年化 10.86%→11.36%，夏普持平。20 个单元测试 | 让策略统计上站得住 |
| 8 | ✅ 2026-07-21 (0e691b3) | 清理 | services/ 封存（新增 `services/README.md` 声明不维护+指回主路径，代码保留作参考骨架不删）；删 `_load_financials` 死代码（78 行伪造 ROE 函数，无调用方）及两处注释引用；根目录 9 个 log（~3.4MB）+ 空 run.sh 删除并进 .gitignore（`/optimizer_*.log` 等根目录模式）；README 重写架构节（三子系统微服务叙事 → research/ 研究链主路径 + 技术栈表更新为 westock/Parquet 仓/IC 门禁现状）。（可选）撮合改次日开盘/VWAP **未做**：paper trading 双账本已在量化「收盘撮合乐观假设」的真实偏差，等模拟盘攒出数据再决定是否值得改引擎。全量 313 passed / 23 skipped | 减少维护幻觉，收尾 |

> **🎉 路线图全部 8 项已于 2026-07-21 完成。** 后续常态化运维事项：
> ① 每日收盘后 `python scripts/update_data.py update --config <池配置>`（增量行情+攒成分快照）；
> ② 每日 `paper_trading signal`/`settle`，每月在本文档追加对账摘要；
> ③ westock 股本接口恢复后补跑 `factor_ic.py`（pb/pe/peg 等 6 个因子截面为 0，见第 5 项残留）；
> ④ westock 板块搜索恢复后核对 `strategy_multi_sector_westock.yaml` 板块码并跑扩宇宙回测；
> ⑤ 成分快照攒够历史后，把 `universe.py` 接入 `warehouse.constituents_asof`（见第 6 项残留）。
